...
$sql = 'insert into `Requests` (student_id, request_type_id, comments, evidence, evidence_ext, career_change_id, campus_change_id, period_id)
values(?, ?, ?, ?, ?, ?, ?, ?);';
$result = $conn->execute_query($sql,[$id, $request_id, $comments, $evidence, $evidence_ext, $career_change_id, $campus_change_id, $row['period_id']]);
echo json_encode([
    "status"=>true,    
    "message"=>"Created"]); 